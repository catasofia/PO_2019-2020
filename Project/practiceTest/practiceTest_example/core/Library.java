...
public class Library implements Serializable {
  ...
  String performSearchRequest(int iDObra) {
    String search = "";
    Work u = getWork(iDObra);
    List<User> users = new ArrayList<>();
    for (Request i : _requests.values())
      if (i.getWork().equals(u))
        users.add(i.getUser());
    ...
    for (int i = 0; i < users.size(); i++) {
      search += users.get(i).showUser();
    }
    return search;
  }

  String pesquisaUser(String term) {
    Integer flag = 0;
    String search = "";
    for (int i = 0; i < _nextUserId; i++) {
      if (getUser(i) != null) {
        String user = _users.get(i).getName() + " " + _users.get(i).getEmail();
        if (user.toLowerCase().contains(term.toLowerCase())) {
          search += _users.get(i).getName()+"\n";
          flag++;
        }
      }
    }
